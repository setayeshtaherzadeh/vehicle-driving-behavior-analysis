...
def correlate_with_fuel(summary):
    """
    Check whether trips with a higher harsh-event rate also show higher
    fuel consumption. Also checks average speed against both variables,
    since speed is a likely confound: highway trips run the engine
    continuously (higher fuel/min) but involve fewer sudden braking/
    acceleration events than stop-and-go city driving.
    """
    corr_harsh_fuel = summary["harsh_event_rate_per_1000"].corr(
        summary["avg_fuel_usage_ml_min"]
    )
    corr_speed_harsh = summary["avg_speed"].corr(summary["harsh_event_rate_per_1000"])
    corr_speed_fuel = summary["avg_speed"].corr(summary["avg_fuel_usage_ml_min"])

    return {
        "harsh_events_vs_fuel": corr_harsh_fuel,
        "avg_speed_vs_harsh_events": corr_speed_harsh,
        "avg_speed_vs_fuel": corr_speed_fuel,
    }
...
def main():
    ...
    correlations = correlate_with_fuel(summary)
    print("\nCorrelations:")
