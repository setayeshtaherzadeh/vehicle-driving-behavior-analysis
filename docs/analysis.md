...
This was calculated separately within each trip (`segment_id`), so that the last reading of one trip is never compared against the first reading of the next trip.

Resulting acceleration values ranged from **-22.25** (hardest braking) to **10.12** (hardest acceleration), with a mean close to 0 — expected, since braking and accelerating roughly cancel out over a full trip.
...
Readings were grouped by trip (`segment_id`) to compute, for each of the 22 trips: total readings, harsh event count, harsh event rate per 1,000 readings, average fuel usage, and average speed. This makes trips comparable to each other despite differing lengths.

## 4. Relationship with fuel consumption

As a validation check, the harsh event rate was compared against average fuel usage across trips.
...
**Interpretation:** the raw fuel-vs-harshness relationship appears to be confounded by driving context. Trips with higher average speed (highway-style driving) show more sustained engine load and higher fuel use per minute, but fewer harsh events (smoother, more continuous driving). Trips with lower average speed (stop-and-go city driving) show more harsh events but lower fuel use per minute, since the engine spends more time idling or at low RPM. In other words, harsh event rate alone is not a reliable predictor of fuel consumption — driving context matters more.
