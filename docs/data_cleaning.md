...
Data was sorted by period (`segment_id`) and then by timestamp, since later analysis (e.g. detecting harsh braking) depends on rows being in correct chronological order within each period.
